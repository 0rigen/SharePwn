...
def geturlcode(target, link=None, urllist=None):
    ...
    if link is not None:
        ...
        if r is not None:
            ...
            success_code = re.match("[2**]", status_code)                          # true success, 2xx
            ...
            if success_code is not None:
                res.append(page)
            if forbid_code is not None:                                       # Add to the results list if
                res.append(page)                                              # any kind of success was returned
            if unauth_code is not None:
                res.append(page)

            logging.info("* Requesting %s => [Returned " % l + str(r.status_code)+"]")
            sys.stdout.flush()
            print("\r[*] Found %s <Code %s>" % (page, str(r.status_code)))
